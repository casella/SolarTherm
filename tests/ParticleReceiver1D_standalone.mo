model ParticleReceiver1D_standalone "Falling particle flow and energy model"
	...
	//Discretisation
		parameter Integer N = 20 "Number of height positions";
	...
	SI.Temperature T_s[N] (start = fill(T_inlet_des,N)) "Curtain Temperature";
	SI.SpecificEnthalpy h_s[N] (start = fill(h0,N)) "Curtain enthalpy";
	...
	SI.Temperature T_w[N+1] (start = fill(T_inlet_des,N+1)) "Receiver wall temperature";
...
equation
	...
	for i in 1:(N) loop
		...
		// Curtain-wall radiation Heat Transfer fluxes
		gc_f[i] = q_solar;
