...
model reference_2
  ...
  SI.Power Ex_des_rate "Exergy destrcution rate";
  SI.Energy Ex_loss(final start=0,displayUnit="MWh");
  SI.Efficiency eff_II "Exergetic efficiency";
  ...
  Models.Sources.SolarModel.Sun       sun(
    ...
    redeclare function solarPosition =
        Models.Sources.SolarFunctions.PSA_Algorithm)
            ...
            {-62,80}})));
  Models.CSP.CRS.HeliostatsField.HeliostatsField_2 heliostatsField(
    n_h=6377,
    lon=data.lon,
    lat=data.lat,
  ...
  Modelica.Blocks.Sources.RealExpression Tamb_input(y=data.Tdry)   annotation(Placement(transformation(extent={{140,70},
            {120,90}})));
  Models.Storage.Tank.Tank_2 tankHot(
    redeclare package Medium = Media.MoltenSalt.MoltenSalt_base,
  ...
  Models.Fluid.Pumps.PumpSimple_2 pumpHot(redeclare package Medium =
        Media.MoltenSalt.MoltenSalt_base)                                                            annotation(Placement(transformation(extent={{66,38},
            {78,50}})));
  Models.Storage.Tank.Tank_2 tankCold(
    redeclare package Medium = Media.MoltenSalt.MoltenSalt_base,
  ...
  Models.Fluid.Pumps.PumpSimple_2 pumpCold(redeclare package Medium =
  ...
  Models.PowerBlocks.PowerBlockModel_2
                                  powerBlock(
    enable_losses=true,
...
equation
  ...
  connect(tankCold.T_amb, powerBlock.T_amb) annotation (Line(
      ...
      color={0,0,127},
      pattern=LinePattern.Dot));
  connect(receiver.Tamb, heliostatsField.T_amb) annotation (Line(
      points={{-28,36.04},{-28,60},{-63.8,60},{-63.8,36.5}},
      color={0,0,127},
      pattern=LinePattern.Dot));
  connect(tankHot.T_amb, pumpHot.T_amb) annotation (Line(
      points={{21.9,73.7},{21.9,80},{72,80},{72,49.16}},
      color={0,0,127},
      pattern=LinePattern.Dot));  
  connect(tankCold.T_amb, pumpCold.T_amb) annotation (Line(
      points={{49.5,-8.3},{4,-8.3},{4,-18.84}},
      color={0,0,127},
      pattern=LinePattern.Dot));   
  ...
  Ex_des_rate=heliostatsField.Ex_des+receiver.Ex_des+tankHot.Ex_des+
  pumpHot.Ex_des+tankCold.Ex_des+pumpCold.Ex_des+powerBlock.Ex_des;
    
  der(Ex_loss)=Ex_des_rate;
  eff_II=(powerBlock.E_net/max(heliostatsField.Ex_solar,0.001))*100;
