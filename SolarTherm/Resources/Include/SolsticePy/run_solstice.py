...
def run_simul(inputs={}):
    ...
    if os.path.exists(tablefile):    
        ...
        if (A_land==0):    
            ...
            output_matadata_motab(table=oelt, field_type=pm.field_type, aiming='single', n_helios=crs.n_helios, A_helio=A_helio, eff_design=crs.eff_des, H_rcv=pm.H_rcv, W_rcv=pm.W_rcv, H_tower=pm.H_tower, lat=pm.lat, A_land=A_land, savedir=tablefile)
            end=time.time()
            print ''
