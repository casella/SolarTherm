...
def output_matadata_motab(table, field_type, aiming, n_helios, A_helio, eff_design, H_rcv, W_rcv, H_tower, lat, A_land, savedir=None):
    '''
    output the .motab table fiel
    '''
    f=open(savedir, 'w')
    ...
    f.write('#METALABELS,n_helios,A_helio,Eff_design,H_rcv,W_rcv,H_tower,lat,A_land\n')
    f.write('##METAUNITS,real,m2,real,m,m,m,deg,m2\n')
    f.write('#METADATA,%s,%s,%s,%s,%s,%s,%s,%s\n'%(n_helios,A_helio,eff_design,H_rcv,W_rcv,H_tower,lat,A_land))
