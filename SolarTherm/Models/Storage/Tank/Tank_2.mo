...
//protected
 ...
 // SI.Temperature T=Medium.temperature(medium.state);
  ...
  SI.SpecificEntropy s=Medium.specificEntropy(medium.state);
...
equation
 ...
 //der(m*((medium.h-h_dead)-T_amb*(s-s_dead)))=Ex_in-Ex_out+W_loss-Ex_des;
 der(m)*((medium.h-h_dead)-T_amb*(s-s_dead))+(der(medium.h)-T_amb*der(s))*m = Ex_in-Ex_out+W_loss-Ex_des;
