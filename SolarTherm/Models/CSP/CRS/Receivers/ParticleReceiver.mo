...
model ParticleReceiver
	...
	parameter SI.Area a = pi*(d_p/2)^2 "cross sectional particle area";

	// Medium properties
	parameter SI.Density rho = 3300 "particle density";
	parameter Real phi_max = 0.6 "packing limit";
	parameter SI.SpecificHeatCapacity cp_s = 1200 "particle specific heat capacity";

	// Environmental variables
	parameter SI.Temperature T_ref = from_degC(25) "Reference temperature";
	parameter SI.CoefficientOfHeatTransfer h_c = 100 "Convective heat transfer coefficient";
	
	//Wall properties
	parameter SI.Efficiency eps_w = 0.8 "receiver wall emissivity";
	parameter SI.ThermalConductivity k_w = 0.2 "b wall thermal conductivity";
	parameter SI.Length th_w = 0.05 "b wall thickness";
	...
	parameter SI.Length dx = H_rcv/(Nx-1) "Vertical step size";
	...
	parameter SI.Velocity vp_in = 0.25 "Particle velocity at the inlet";
	SI.Velocity vp[Nx-1] "Particle velocity";
	...
	//SI.Efficiency eps_c[Nx-1] "particle emissivity across the receiver height";
	//SI.Efficiency tau_c[Nx-1] "particle tramittance across the receiver height";
	//SI.Efficiency abs_c[Nx-1] "particle absorptance across the receiver height";
	...
	SI.HeatFlux q_solar "Uniform solar flux";
