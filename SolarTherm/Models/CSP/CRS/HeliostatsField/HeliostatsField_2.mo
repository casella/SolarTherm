...
model HeliostatsField_2
  extends Interfaces.Models.Heliostats;
  parameter SI.Temperature T_sun = 5777 "Sun temperature";
  ...
  SI.Energy Ex_solar(final start=0);
    
  Modelica.Blocks.Interfaces.RealInput T_amb annotation(Placement(
  		transformation(extent = {{-20, -20}, {20, 20}},
  		rotation = -90,
  		origin = {30, 110}), iconTransformation(extent = {{-110, -72}, {-86, -48}}, rotation = -90,origin = {110, 0})));
...
//   optical.dec=solar.dec;
  ...
  Ex_in = solar.dni*(n_h*A_h)*(1-(4/3)*(T_amb/T_sun)+(1/3)*((T_amb/T_sun)^4));
  Ex_out = Q_net*(1-(4/3)*(T_amb/T_sun)+(1/3)*((T_amb/T_sun)^4));
