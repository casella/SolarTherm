...
package OnDesign "On-design calculation of cycles"
...
//  eta_carnot = 1 - T_amb / T_high;
    ...
    model simpleRecupCycleHeater
      ...
      replaceable package MedPB = SolarTherm.Media.CO2.CO2_ph;
      //Parameters
      parameter SI.AbsolutePressure p_high = CV.from_bar(250) "high pressure of the cycle";
      parameter SI.ThermodynamicTemperature T_high = CV.from_degC(715) "inlet temperature of the turbine";
      parameter SI.ThermodynamicTemperature T_amb = CV.from_degC(30) "ambiant temperature";
...
// Exergy analysis
    ...
    annotation(
      Documentation(info = "<html>
  		<p>This section contains different on-design cycles. </p>
  		<p>The mass flow in the overall cycle is imposed by the nominal power. </p>
  		<p>It can be useful to investigate different on-design parameters</p>
  		</html>"));
  end Cycles;
...
//deltaT_TLMD= if deltaT[1] * deltaT[N_q] <= 0 then (abs(deltaT[1]) ^ (1 / 3) * sign(deltaT[1]) / 2 + abs(deltaT[N_q]) ^ (1 / 3) * sign(deltaT[N_q]) / 2) ^ 3 else (deltaT[1] - deltaT[N_q]) / (Modelica.Math.log(deltaT[1] / deltaT[N_q]) + 0.0001);
  ...
  <p> The conductance in each sub-HX is calculated with Q=UA*DTAverage, DTAverage being the average between the inlet and outlet delta T. As the overall HX is discretized, results are close. </p>
  <p> On the number of discretization, little difference is observed between 15 and 60 sub-HX. </p>
  ...
  extends SolarTherm.Media.CO2.PropCO2;
    ...
    SolarTherm.Types.SpecificExergy ex_d "destroyed exergy";
    parameter Boolean is_first_heater = true "if a second heater is introduced, there is a need for one more mass flow equality";
  equation
    state_a = MedPB.setState_phX(port_a.p, inStream(port_a.h_outflow));
  ...
  if not is_first_heater then 
    ...
    annotation(
      Documentation(info = "<html>
  		<p> This heater is useful to perform quicker on-design analysis of the cycles, without caring about the HTF. </p>
  		</html>"));
  end Heater;
  ...
  equation
    ...
    for i in 1:N_cool - 1 loop
      ...
      Q_dis = -UA_dis[i] * (deltaT[i]+deltaT[i+1])/2;
    ...
    annotation(
      Documentation(info = "<html>
  		<p> The overall conductance UA of the cooler is calculated by discretizing and applying an average mean temperature to every sub-HX. </p>
  		<p>The Cp is varying from 4kJ/kg.K to 1.2 kJ/kg.K in this temperature/pressure range.  </p>
  		<p> The outlet temperature is fixed by the user. The electrical power necessary for the fans is estimated by  P_elec/(Q_cooler*deltaT_average)=constante, for any sCO2/air HX. P_elec is the power to run the fans, Q_cooler is the energy exchanged with outside and deltaT_average is the average of T_CO2-T_amb, the average is taken between the outlet and the inlet. It is in general 1% of the nominal power.</p>
  		<p> The above constant value (but not model) come from: </p> <p>Sienicki, James J., Moisseytsev, Anton, and Lv, Qiuping. Dry Air Cooling and the sCO2 Brayton Cycle. Proceedings of the ASME Turbo Expo 2017: Turbomachinery Technical Conference and Exposition. Volume 9: Oil and Gas Applications; Supercritical CO2 Power Cycles; Wind Energy. Charlotte, North Carolina, USA. June 26–30, 2017. V009T38A015. ASME. https://doi.org/10.1115/GT2017-64042  </p>
          <p>A better model from the article: S. R. Pidaparti, C. W. White, A. C. O’Connel, et N. T. Weiland, « Cooling system cost and performance models for economic sCO2 plant optimization of cooling with respect to cold sCO2 temperature », oct. 2019. is available. </p>
          <p> See:
          <ul>
  <li>https://netl.doe.gov/energy-analysis/details?id=3746 : paper</li>
  <li>https://netl.doe.gov/energy-analysis/details?id=3199 : documentation</li>
  <li>https://netl.doe.gov/energy-analysis/details?id=3866 : direct dry cooling </li>
  </ul>
...
// Compressor on-design parameters to be calculated
  ...
  annotation(
      Documentation(info = "<html>
  		<p>This section proposes on-design calculation of sCO2 cycles. In the Cycles package, different cycles can be found. </p>
  		<p>The on-design style is to use the equation section, in order to understand what is happening. </p>
