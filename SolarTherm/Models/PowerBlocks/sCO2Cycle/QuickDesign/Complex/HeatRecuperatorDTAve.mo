within SolarTherm.Models.PowerBlocks.sCO2Cycle.QuickDesign.Complex;
 ...
 model HeatRecuperatorDTAve "The heat recuperator is subdivised in N_q segments in order to accurately represent the CO2 properties variation."
      ...
      //On-design
      parameter SI.MassFlowRate[N_points + 1] m_flow_turb(each fixed = false, each start= P_nom/10^5);
      parameter SI.MassFlowRate[N_points + 1] m_flow_comp(each fixed = false, each start= P_nom/10^5);
