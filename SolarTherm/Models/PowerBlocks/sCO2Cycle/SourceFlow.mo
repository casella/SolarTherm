...
model SourceFlow
  ...
  SI.ThermodynamicTemperature T_b;
  Real TEST2;
...
equation
  state_b=MedPB.setState_pTX(p_out,(T_out-273.15)*(0.8+0.4*time)+273.15);
  port_b.h_outflow=MedPB.specificEnthalpy(state_b);
  T_b=MedPB.temperature(state_b);
  TEST2=inStream(port_b.h_outflow);
  if use_m_parameter then 
  port_b.m_flow = -max(m_flow,m_flow*0.0001);
  end if;
  port_b.p = state_b.p;
end SourceFlow;