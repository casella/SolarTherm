...
model PhysicalParticleCO2
  ...
  // Receiver
  Models.CSP.CRS.Receivers.ParticleReceiver receiver(redeclare package Medium = Medium, em = em_curtain, ab = ab_curtain, H_rcv = H_receiver, W_rcv = W_receiver, L_rcv = L_receiver, const_alpha = true, alpha = h_th_rec) annotation(
  ...
  // ReceiverControl
  Models.Control.ReceiverControl controlCold(T_ref = T_hot_set, m_flow_min = m_flow_rec_min, m_flow_max = m_flow_rec_max, y_start = m_flow_rec_start, L_df_on = cold_tnk_defocus_lb, L_df_off = cold_tnk_defocus_ub, L_off = cold_tnk_crit_lb, L_on = cold_tnk_crit_ub, Ti = Ti, Kp = Kp) annotation(
    Placement(transformation(extent = {{24, -10}, {10, 4}})));
  // Power block
  Models.PowerBlocks.sCO2Cycle.DirectDesign.recompPB powerBlock(redeclare package MedRec = Medium, P_gro = P_gross, T_HTF_in_des = T_in_ref_blk,T_amb_des=blk_T_amb_des, T_low = T_comp_in, external_parasities = false, nu_min = nu_min_blk) annotation(
