...
model SimpleParticleCO2 "SimpleParticle with a CO2 power block added"
...
allowFlowReversal=false) "System props and default values";
  ...
  // Can provide details of modelling accuracy, assumptions and initialisation
	...
	parameter Medium.Temperature T_cold_set (fixed=false) "Target cold tank temperature";
	parameter Medium.Temperature T_hot_set = CV.from_degC(730) "Target hot tank temperature";

	parameter Medium.Temperature T_cold_start = T_cold_set "Cold tank starting temperature";
	parameter Medium.Temperature T_hot_start = T_hot_set "Hot tank starting temperature";
  ...
  parameter SI.ThermodynamicTemperature T_HTF_in_des = T_hot_set "inlet temperature of the particles";
  parameter Integer N_exch = 3;
	...
	parameter SI.MassFlowRate m_flow_fac =  (ab_curtain*eff_opt*dni_des*A_field -(em_curtain*5.67*10^(-8)*((((T_cold_set + T_hot_set)/2))^4-rec_T_amb_des^4)+h_th_rec*(((T_cold_set + T_hot_set)/2)-rec_T_amb_des))*A_rec)/(cp_set*(T_hot_set-T_cold_set)) "Mass flow rate to receiver at design point";
	...
	// Price of the extractor and power block are included in the power block
	...
	//Price of the extractor is included in the power block
	...
	SolarTherm.Models.CSP.CRS.HeliostatsField.SwitchedCL_2 CL(redeclare model OptEff=SolarTherm.Models.CSP.CRS.HeliostatsField.FileOE(
...
//redeclare model OptEff=SolarTherm.Models.CSP.CRS.HeliostatsField.IdealIncOE(alt_fixed=45),
    ...
    SolarTherm.Models.PowerBlocks.sCO2Cycle.QuickCO2PB.ComplicatedPB.simpleRecupPB PB(
        redeclare package MedRec=Medium,
        T_high=T_high,
        T_HTF_in_des=T_HTF_in_des,
        p_high=p_high,
...
//m_HTF_des=m_flow_blk,
	...
	Boolean T_amb_good "True if outside temperature inferior to inlet compressor temperature-5";
    //SI.MassFlowRate controled_m (start=m_flow_blk) "Control equation for a constant HTF outlet temperature";
...
initial equation
  ...
  C_block=PB.C_PB;
  C_cap = ((C_field + C_site + C_tower + C_lift + C_receiver + C_storage + C_block + C_bop) * (1+r_contg)) * (1+r_indirect) * (1+r_cons) + C_land;
  T_cold_set = PB.T_HTF_out_des;

equation
	connect(wea.wbus, CL.wbus);
	connect(wea.wbus, RC.wbus);
	connect(wea.wbus.Tdry, PB.T_amb);
	...
	connect(lift_ext.port_b, PB.fluid_a);

	connect(PB.fluid_b, lift_stc.port_a);
	connect(lift_stc.port_b, STC.port_a);


	connect(PB.W_net, par.P_gen);
    ...
    T_amb_good = wea.wbus.Tdry <= T_low-5;
    ...
    //controled_m = PB.exchanger.m_HTF_des*(PB.exchanger.h_in_HTF_des-PB.exchanger.h_out_HTF_des)/(PB.exchanger.h_HTF[N_exch]-PB.exchanger.h_out_HTF_des);
	lift_ext.m_flow_set = if fill_ctnk and not empty_htnk  then m_flow_blk else 0;

	CL.track = true;

	P_elec = PB.W_net - (par.P_par + par_fix_fr*P_net + lift_rec.W + lift_ext.W + lift_stc.W); //TODO: add parasitic losses in the heliostat field and tanks
	...
	annotation(experiment(StartTime = 0,  StopTime = 34000000, Interval = 7200, Tolerance = 1e-05),
    __OpenModelica_simulationFlags( outputFormat = "mat", s = "dassl"));
    annotation(
      Documentation(info = "<html>
  		<p>Regarding the power block, please refer to the documentation well furnished of Model.sCO2Cycle. Off-design model is performed through interpolation of a few points regarding the TIT. </p>
  		</html>"));
end SimpleParticleCO2;